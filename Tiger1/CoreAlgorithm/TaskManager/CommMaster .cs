...
namespace CoreAlgorithm.TaskManager
{
    ...
    { 
        ...
        public Double GetMsgID(int ID,int compare)
        {
            string sql = "";
            Double Id = 0;
            try
        ...
        public void Send_SignsMessage()
        {
            ...
            try
            {
                ...
                if(Program.MessageFlg ==4)
                {
                    MAXRECID = GetMsgID(21);
                }
                ...
                if(MAXRECID!=-1)
                { 
                    sql = string.Format("select SEQ_SEND,ID_LOT_PROD,ID_PART_LOT,NAME_PROD,WT_AVG_LEN_PROD,DES_FIPRO_SECTION,DIM_LEN,NUM_BAR,L3ACK from TLabelContent WHERE REC_ID={0}", MAXRECID);
                    string SEQ_SEND = "",ID_LOT_PROD = "", NAME_PROD="", DES_FIPRO_SECTION="";    
                    Int16 ID_PART_LOT = 0, NUM_BAR=0, NAME_PROD_NUM=0, L3ACK=0;
                    ...
                    for (int i = 0; i<dt.Rows.Count; i++)
                    {
                        ...
                        L3ACK= Int16.Parse(dt.Rows[i]["L3ACK"].ToString());
                    }
                    ...
                    byte[] sendArray = Enumerable.Repeat((byte)0x0, 80).ToArray();
                    ...
                    byte[] byteArray9 = BitConverter.GetBytes(L3ACK);//8
                    ...
                    Buffer.BlockCopy(byteArray9, 0, sendArray, 78, byteArray9.Length);


                    if (sendArray.Length > 0)
                        {
                            PLCSocketServer.senddata(sendArray);
                            string str = "二级发送消息到虎踞PLC"+ SEQ_SEND + " " + ID_LOT_PROD + " "  + ID_PART_LOT.ToString() + " " + DIM_LEN .ToString()+ " "+ NUM_BAR+ " " + L3ACK;
