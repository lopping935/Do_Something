...
namespace AnBRobotSystem
{
    public partial class MdiParent : Form
    {
        ...
        private void MdiParent_Load(object sender, EventArgs e)
        {
            ...
            if (CoreAlgorithmRun == false)
            {
                wsm = new WinServerManage(new Hashtable(),
                                                          "D:\\MAnBRobotSetup\\Debug\\MCoreAlgorithm.exe",
                                                          "MAnB_CoreAlgorithm");
            ...
            {
                wsm = new WinServerManage(new Hashtable(),
                                                          "D:\\MAnBRobotSetup\\Debug\\MCoreAlgorithmMES.exe",
                                                          "MAnB_CoreAlgorithmMES");
                th = new System.Threading.Thread(new System.Threading.ThreadStart(wsm.StartService));
                th.Start();
        ...
        {
            listView_Server.Items[0].Tag = "D:\\MAnBRobotSetup\\Debug\\MCoreAlgorithm.exe";
            //listView_Server.Items[1].Tag = appPath + "\\Server\\DataAcquisition.exe";
            listView_Server.Items[1].Tag = "D:\\MAnBRobotSetup\\Debug\\MCoreAlgorithmMES.exe";

            //listView_Server.Items[3].Tag = appPath + "\\Server\\Ice.exe";
            WinServerManage wsm = new WinServerManage();
            if (wsm.ServiceIsExisted("MAnB_CoreAlgorithm")) serverState.Add("MAnB_CoreAlgorithm", wsm.getServiceState("MAnB_CoreAlgorithm"));
            if (wsm.ServiceIsExisted("MAnB_CoreAlgorithmMES")) serverState.Add("MAnB_CoreAlgorithmMES", wsm.getServiceState("MAnB_CoreAlgorithmMES"));
        }
        ...
        private void MenuStrip_Server_Init_Click(object sender, EventArgs e)
        {
            ...
            lvi.SubItems.Add(listView_Server.SelectedItems[0].SubItems[1].Text);
            this.listView1.Items.Add(lvi);
            if (listView_Server.SelectedItems[0].SubItems[1].Text == "MAnB_CoreAlgorithm")
            {
                serverState["MAnB_CoreAlgorithm"] = ServiceControllerStatus.StartPending;
            }
            if (listView_Server.SelectedItems[0].SubItems[1].Text == "MAnB_CoreAlgorithmMES")
            {
                serverState["MAnB_CoreAlgorithmMES"] = ServiceControllerStatus.StartPending;
            }
        ...
        {
            WinServerManage wsm = new WinServerManage();
            if (wsm.ServiceIsExisted("MAnB_CoreAlgorithm"))
            {
                if (wsm.getServiceState("MAnB_CoreAlgorithm") != serverState["MAnB_CoreAlgorithm"])
                {
                    ListViewItem lvi = new ListViewItem();
                    lvi.Text = DateTime.Now.ToString();
                    lvi.SubItems.Add("通知");
                    lvi.SubItems.Add(wsm.getServiceState("MAnB_CoreAlgorithm").ToString());
                    lvi.SubItems.Add("MAnB_CoreAlgorithm服务更改成功！");
                    this.listView1.Items.Add(lvi);
                    serverState["MAnB_CoreAlgorithm"] = wsm.getServiceState("MAnB_CoreAlgorithm");
                    ...
                    Log.addLog(log, LogType.INFO, "MAnB_CoreAlgorithm服务更改为：" + serverState["MAnB_CoreAlgorithm"].ToString());
                }
            }
            if (wsm.ServiceIsExisted("MAnB_CoreAlgorithmMES"))
            {
                if (wsm.getServiceState("MAnB_CoreAlgorithmMES") != serverState["MAnB_CoreAlgorithmMES"])
                {
                    ListViewItem lvi = new ListViewItem();
                    lvi.Text = DateTime.Now.ToString();
                    lvi.SubItems.Add("通知");
                    lvi.SubItems.Add(wsm.getServiceState("MAnB_CoreAlgorithmMES").ToString());
                    lvi.SubItems.Add("MAnB_CoreAlgorithmMES服务更改成功！");
                    this.listView1.Items.Add(lvi);
                    serverState["MAnB_CoreAlgorithmMES"] = wsm.getServiceState("MAnB_CoreAlgorithmMES");
                    log4net.ILog log = log4net.LogManager.GetLogger(MethodBase.GetCurrentMethod().DeclaringType.ToString() + "::" + MethodBase.GetCurrentMethod().ToString());
                    Log.addLog(log, LogType.INFO, "MAnB_CoreAlgorithmMES服务更改为：" + serverState["MAnB_CoreAlgorithmMES"].ToString());
                }
            }                        
        }
        ...
        private void RefrashLog()
        {
             ...
             {           
               ...
               this.listView2.Items[this.listView2.Items.Count - 1].EnsureVisible();
                ...
                {
                    log4net.ILog log = LogManager.GetLogger(MethodBase.GetCurrentMethod().DeclaringType.ToString() + "::" + MethodBase.GetCurrentMethod().ToString());
