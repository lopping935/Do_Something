...
namespace AnBRobotSystem.ChildForm
{
    ...
    public partial class ModelSet : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            string sqltext1 = "select REC_ID 流水号,MACHINE_NO 打包机组号,ID_LOT_PROD 生产批号,ID_PART_LOT 分批号,NUM_BDL 捆号, SEQ_LEN 长度顺序号, SEQ_OPR 操作顺序号, DIM_LEN 米长, IND_FIXED 定尺标志, SEQ_SEND 下发顺序号, NUM_BAR 捆内支数, SEQ_LIST 排列序号, LA_BDL_ACT 重量, NO_LICENCE 许可证号, NAME_PROD 产品名称, NAME_STND 执行标准, ID_HEAT 熔炼号, NAME_STLGD 钢牌号, DES_FIPRO_SECTION 断面规格描述, ID_CREW_RL 轧制班别, ID_CREW_CK 检查班别, TMSTP_WEIGH 生产日期, BAR_CODE 条码内容, NUM_HEAD 头签个数, NUM_TAIL 尾签个数,L3TMSTP_SEND MES发送时刻,IMP_FINISH 状态信息,REC_IMP_TIME 状态更新时刻,REC_CREATE_TIME 记录创建时刻";
            string sqltext = string.Format(sqltext1+ " from(select * from [MDBAnBRobotData].[dbo].[TLabelContent] where IMP_FINISH=31 and REC_CREATE_TIME>='{0}' and REC_CREATE_TIME<='{1}'  union all select * from [MDBAnBRobotData].[dbo].[HLabelContent]  where IMP_FINISH=31 and REC_CREATE_TIME>='{0}' and REC_CREATE_TIME<='{1}' )aa order by REC_ID desc", dateTimePicker1.Value.ToString("yyyy-MM-dd HH:mm:ss"), dateTimePicker2.Value.ToString("yyyy-MM-dd HH:mm:ss"));
            ...
            try {
               DataTable dt1 = db.ExecuteDataTable(db.GetSqlStringCommond(sqltext)); 
