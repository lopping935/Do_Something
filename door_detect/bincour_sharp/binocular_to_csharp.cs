...
public partial class HDevelopExport1
{
            ...
            ))-1); hv_Index = (int)hv_Index + 1)
        {
          ...
          try
          {
            ...
            if ((int)(new HTuple(hv_Label.TupleNotEqual(""))) != 0)
            {
              ...
              {
              hv_Ascent.Dispose();hv_Descent.Dispose();hv_TextWidth.Dispose();hv_TextHeight.Dispose();
              HOperatorSet.GetStringExtents(hv_ExpDefaultWinHandle, hv_Sublabels.TupleSelect(
  ...
  // Local procedures 
  public void check_epipolar_constraint (HObject ho_ImageRectifiedL, HObject ho_ImageRectifiedR, 
      HTuple hv_RectCamParL, HTuple hv_RectCamParR, HTuple hv_WindowHandle1, HTuple hv_WindowHandle2, 
      HTuple hv_CaltabFile, out HTuple hv_EpipolarError)
  {
    ...
    HObject ho_CaltabL, ho_CaltabR, ho_EpipolarLine=null;
    HObject ho_Cross;

    // Local control variables 

    HTuple hv_SizeGauss = new HTuple(), hv_MarkThresh = new HTuple();
    HTuple hv_MinDiamMarks = new HTuple(), hv_StartThresh = new HTuple();
    HTuple hv_DeltaThresh = new HTuple(), hv_MinThresh = new HTuple();
    HTuple hv_Alpha = new HTuple(), hv_MinContLength = new HTuple();
    HTuple hv_MaxDiamMarks = new HTuple(), hv_RCoordL = new HTuple();
    HTuple hv_CCoordL = new HTuple(), hv_StartPoseL = new HTuple();
    HTuple hv_RCoordR = new HTuple(), hv_CCoordR = new HTuple();
    HTuple hv_StartPoseR = new HTuple(), hv_Index = new HTuple();
    HTuple hv_String = new HTuple();
    // Initialize local and output iconic variables 
    HOperatorSet.GenEmptyObj(out ho_CaltabL);
    HOperatorSet.GenEmptyObj(out ho_CaltabR);
    HOperatorSet.GenEmptyObj(out ho_EpipolarLine);
    HOperatorSet.GenEmptyObj(out ho_Cross);
    hv_EpipolarError = new HTuple();
    try
    {
      ...
      hv_SizeGauss.Dispose();
      hv_SizeGauss = 3;
      hv_MarkThresh.Dispose();
      hv_MarkThresh = 120;
      hv_MinDiamMarks.Dispose();
      hv_MinDiamMarks = 5;
      hv_StartThresh.Dispose();
      hv_StartThresh = 128;
      hv_DeltaThresh.Dispose();
      hv_DeltaThresh = 10;
      hv_MinThresh.Dispose();
      hv_MinThresh = 18;
      hv_Alpha.Dispose();
      hv_Alpha = 0.9;
      hv_MinContLength.Dispose();
      hv_MinContLength = 15;
      hv_MaxDiamMarks.Dispose();
      hv_MaxDiamMarks = 100;

      //Find the calibration points and the correspondences of these points in both images:
      ho_CaltabL.Dispose();
      HOperatorSet.FindCaltab(ho_ImageRectifiedL, out ho_CaltabL, hv_CaltabFile, 
          hv_SizeGauss, hv_MarkThresh, hv_MinDiamMarks);
      hv_RCoordL.Dispose();hv_CCoordL.Dispose();hv_StartPoseL.Dispose();
      HOperatorSet.FindMarksAndPose(ho_ImageRectifiedL, ho_CaltabL, hv_CaltabFile, 
          hv_RectCamParL, hv_StartThresh, hv_DeltaThresh, hv_MinThresh, hv_Alpha, 
          hv_MinContLength, hv_MaxDiamMarks, out hv_RCoordL, out hv_CCoordL, out hv_StartPoseL);
      ho_CaltabR.Dispose();
      HOperatorSet.FindCaltab(ho_ImageRectifiedR, out ho_CaltabR, hv_CaltabFile, 
          hv_SizeGauss, hv_MarkThresh, hv_MinDiamMarks);
      hv_RCoordR.Dispose();hv_CCoordR.Dispose();hv_StartPoseR.Dispose();
      HOperatorSet.FindMarksAndPose(ho_ImageRectifiedR, ho_CaltabR, hv_CaltabFile, 
          hv_RectCamParR, hv_StartThresh, hv_DeltaThresh, hv_MinThresh, hv_Alpha, 
          hv_MinContLength, hv_MaxDiamMarks, out hv_RCoordR, out hv_CCoordR, out hv_StartPoseR);
      hv_EpipolarError.Dispose();
      using (HDevDisposeHelper dh = new HDevDisposeHelper())
      {
      hv_EpipolarError = ((((hv_RCoordR-hv_RCoordL)).TupleAbs()
          )).TupleMax();
      }
      ...
      using (HDevDisposeHelper dh = new HDevDisposeHelper())
      {
      HOperatorSet.SetPart(hv_ExpDefaultWinHandle, 0, 0, (hv_RectCamParL.TupleSelect(
          7))-1, (hv_RectCamParL.TupleSelect(6))-1);
      }
      HOperatorSet.DispObj(ho_ImageRectifiedL, hv_ExpDefaultWinHandle);
      for (hv_Index=6; (int)hv_Index<=49; hv_Index = (int)hv_Index + 7)
      {
        //Display the epipolar line of a certain point
        using (HDevDisposeHelper dh = new HDevDisposeHelper())
        {
        ho_EpipolarLine.Dispose();
        HOperatorSet.GenContourPolygonXld(out ho_EpipolarLine, ((((hv_RCoordL.TupleSelect(
            hv_Index))).TupleRound())).TupleConcat(((hv_RCoordL.TupleSelect(hv_Index))).TupleRound()
            ), (new HTuple(0)).TupleConcat((hv_RectCamParL.TupleSelect(6))-1));
        }
        HOperatorSet.DispObj(ho_EpipolarLine, hv_ExpDefaultWinHandle);
      }
      ho_Cross.Dispose();
      HOperatorSet.GenCrossContourXld(out ho_Cross, hv_RCoordL, hv_CCoordL, 6, 0);
      ...
      disp_message(hv_ExpDefaultWinHandle, hv_String, "window", 180, 12, "white", 
          "false");
      //dev_set_window(...);
      HOperatorSet.SetColor(hv_ExpDefaultWinHandle, "yellow");
      HOperatorSet.ClearWindow(hv_ExpDefaultWinHandle);
      using (HDevDisposeHelper dh = new HDevDisposeHelper())
      {
      HOperatorSet.SetPart(hv_ExpDefaultWinHandle, 0, 0, (hv_RectCamParR.TupleSelect(
          7))-1, (hv_RectCamParR.TupleSelect(6))-1);
      }
      HOperatorSet.DispObj(ho_ImageRectifiedR, hv_ExpDefaultWinHandle);
      for (hv_Index=6; (int)hv_Index<=49; hv_Index = (int)hv_Index + 7)
      {
        //display the epipolar line of a certain point
        using (HDevDisposeHelper dh = new HDevDisposeHelper())
      ...
      ho_Cross.Dispose();
      HOperatorSet.GenCrossContourXld(out ho_Cross, hv_RCoordR, hv_CCoordR, 6, 0);
      HOperatorSet.DispObj(ho_Cross, hv_ExpDefaultWinHandle);
      ho_CaltabL.Dispose();
      ho_CaltabR.Dispose();
      ho_EpipolarLine.Dispose();
      ...
      hv_RCoordL.Dispose();
      hv_CCoordL.Dispose();
      hv_StartPoseL.Dispose();
      hv_RCoordR.Dispose();
      hv_CCoordR.Dispose();
      hv_StartPoseR.Dispose();
      hv_Index.Dispose();
      hv_String.Dispose();
  ...
  public void visualize_reconstruction_result (HObject ho_ImageMappedL, HObject ho_ImageMappedR, 
      HTuple hv_CamParamRectL, HTuple hv_CamParamRectR, HTuple hv_RelPoseRect, HTuple hv_WindowHandleL, 
      HTuple hv_WindowHandleR)
  {
    ...
    HObject ho_Disparity, ho_Score, ho_X, ho_Y;
    HObject ho_Z;

    // Local control variables 

    HTuple hv_MinDisparity = new HTuple(), hv_MaxDisparity = new HTuple();
    HTuple hv_ObjectModel3D = new HTuple(), hv_ObjectModel3DThresholded = new HTuple();
    HTuple hv_Visualize3DPoseOut = new HTuple();
    // Initialize local and output iconic variables 
    ...
    try
    {
      hv_MinDisparity.Dispose();
      HOperatorSet.DistanceToDisparity(hv_CamParamRectL, hv_CamParamRectR, hv_RelPoseRect, 
          0.45, out hv_MinDisparity);
      hv_MaxDisparity.Dispose();
      HOperatorSet.DistanceToDisparity(hv_CamParamRectL, hv_CamParamRectR, hv_RelPoseRect, 
          0.55, out hv_MaxDisparity);
      using (HDevDisposeHelper dh = new HDevDisposeHelper())
      {
      ho_Disparity.Dispose();ho_Score.Dispose();
      HOperatorSet.BinocularDisparityMs(ho_ImageMappedL, ho_ImageMappedR, out ho_Disparity, 
          out ho_Score, hv_MinDisparity.TupleInt(), hv_MaxDisparity.TupleInt(), 10, 
          10, new HTuple(), new HTuple());
      }

      //dev_set_window(...);
      HOperatorSet.ClearWindow(hv_ExpDefaultWinHandle);
      HOperatorSet.DispObj(ho_Disparity, hv_ExpDefaultWinHandle);
      HOperatorSet.DispText(hv_ExpDefaultWinHandle, "Disparity image", "window", 
          "top", "left", "black", new HTuple(), new HTuple());

      ho_X.Dispose();ho_Y.Dispose();ho_Z.Dispose();
      HOperatorSet.DisparityImageToXyz(ho_Disparity, out ho_X, out ho_Y, out ho_Z, 
          hv_CamParamRectL, hv_CamParamRectR, hv_RelPoseRect);
      hv_ObjectModel3D.Dispose();
      xyz_attrib_to_object_model_3d(ho_X, ho_Y, ho_Z, ho_ImageMappedL, "&gray", out hv_ObjectModel3D);
      hv_ObjectModel3DThresholded.Dispose();
      HOperatorSet.SelectPointsObjectModel3d(hv_ObjectModel3D, "neighbor_distance 45", 
          0, 0.001, out hv_ObjectModel3DThresholded);
      hv_Visualize3DPoseOut.Dispose();
      visualize_object_model_3d(hv_ExpDefaultWinHandle, hv_ObjectModel3DThresholded, 
          new HTuple(), ((((((new HTuple(-0.0711891)).TupleConcat(-0.0252995)).TupleConcat(
          3.54703)).TupleConcat(356.656)).TupleConcat(359.997)).TupleConcat(1.04651)).TupleConcat(
          0), "color_attrib", "&gray", new HTuple("3D object model, created from disparity image"), 
          new HTuple(), new HTuple(), out hv_Visualize3DPoseOut);
  ...
  private void action()
  {
    ...
    HObject ho_ImageRectifiedL, ho_ImageRectifiedR, ho_DisparityImage;
    HObject ho_Score1, ho_Distance, ho_Score, ho_Regions, ho_ConnectedRegions;
    ...
    HOperatorSet.GenEmptyObj(out ho_DisparityImage);
    HOperatorSet.GenEmptyObj(out ho_Score1);
    ...
    try
    {
      ...
      ho_DisparityImage.Dispose();ho_Score1.Dispose();
      HOperatorSet.BinocularDisparity(ho_ImageRectifiedL, ho_ImageRectifiedR, out ho_DisparityImage, 
          out ho_Score1, "ncc", 11, 11, 0, -340, -228, 1, 0.5, "left_right_check", 
          "interpolation");
      ...
      HOperatorSet.BinocularDistance(ho_ImageRectifiedL, ho_ImageRectifiedR, out ho_Distance, 
          ...
          17, 17, 0, -350, -228, 3, 0.5, "left_right_check", "interpolation");
  ...
  {
    //hv_ExpDefaultWinHandle = Window;HTuple Window
    action();
  }
