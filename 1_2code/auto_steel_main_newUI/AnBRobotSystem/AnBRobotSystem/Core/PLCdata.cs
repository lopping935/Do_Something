...
namespace AnBRobotSystem.Core
{
    public struct plcdata
    {
        ...
        public bool A_ZT_start_signal;
        public bool A_MODEL_RESET;
    ...
    public class PLCdata
    {
        ...
        public async void  set_speed(string flag, Single speed)
        {
            try
            {
                if (flag == "A")
                {
                    ...
                    string sqltext = string.Format("insert into [zt_controldata] VALUES ('{0}','{1}','{2}','{3}','{4}','{5}','{6}')", in_time, speed.ToString(), TB_weight_speed.ToString(), ZT_data.TB_weight.ToString(), "A", ZT_data.GB_A_angle.ToString(),MdiParent.atuomodelA.ZT_program_flag.ToString());
                ...
                else
                {
                    ...
                    in_time = DateTime.Now;
                    string sqltext = string.Format("insert into [zt_controldata] VALUES ('{0}','{1}','{2}','{3}','{4}','{5}','{6}')", in_time, speed.ToString(), TB_weight_speed.ToString(), ZT_data.TB_weight.ToString(), "B", ZT_data.GB_B_angle.ToString(), MdiParent.atuomodel.ZT_program_flag.ToString());
                    db_plc_helper.MultithreadExecuteNonQuery(sqltext);
                    db_plc_helper.MultithreadExecuteNonQuery(sqltext);
        ...
        public void set_speed(string flag)
        {
            try
            {
                ...
                {
                    in_time = DateTime.Now;
                    string sqltext = string.Format("insert into [zt_controldata] VALUES ('{0}','{1}','{2}','{3}','{4}','{5}','{6}')", in_time, "", TB_weight_speed.ToString(), ZT_data.TB_weight.ToString(), "A", ZT_data.GB_A_angle.ToString(),MdiParent.atuomodelA.ZT_program_flag.ToString());
                    db_plc_helper.MultithreadExecuteNonQuery(sqltext);
                ...
                {
                    in_time = DateTime.Now;
                    string sqltext = string.Format("insert into [zt_controldata] VALUES ('{0}','{1}','{2}','{3}','{4}','{5}','{6}')", in_time, "", TB_weight_speed.ToString(), ZT_data.TB_weight.ToString(), "B", ZT_data.GB_B_angle.ToString(),MdiParent.atuomodelA.ZT_program_flag.ToString());
                    db_plc_helper.MultithreadExecuteNonQuery(sqltext);
                }
            }
            catch (Exception e)
        ...
        public async void reset_start(string station)
        {
            if (station == "B")
                await plc300.WriteAsync("DB50.DBX59.3", true);
            if (station == "A")
                await plc300.WriteAsync("DB50.DBX64.1", true);

        }
        public async void write_startsignal(string station)
        {
            if(station=="B")
                await plc300.WriteAsync("DB50.DBX3.5", true);
            if(station=="A")
                await plc300.WriteAsync("DB50.DBX64.0", true);
        }

        public async void write_resetstartsignal(string station)
        {
            if (station == "B")
                await plc300.WriteAsync("DB50.DBX3.5", false);
            if (station == "A")
                await plc300.WriteAsync("DB50.DBX64.0", false);
        }
        //发送任务存活心跳
        ...
        public void updata_RealTime_Car_Bag_plc()
        {
            try
            {
                ...
                if (LGB_A_getpow == false && ZT_data.GB_A_connect == true|| MdiParent.updata_GBA_fulweight == true)//当得电信号上升沿时 写入数据
                {
                    ...
                    if (dr.HasRows)
                    {
                        ...
                        if (number == 1)
                        {
                            ...
                            db_plc_helper.MultithreadExecuteNonQuery(sqltext);
                            writelog("RealTime_Car_Bag", "A道罐包总重写入plc！", "log");
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        db_plc_helper.MultithreadExecuteNonQuery(sqltext);
                        Program.GB_data_flag = 1005;
                        writelog("RealTime_Car_Bag", "A道未查询到总重！", "log");
                    }
                ...
                if (LGB_B_getpow == false && ZT_data.GB_B_connect == true|| MdiParent.updata_GBB_fulweight == true)
                {
                    ...
                    if (dr.HasRows)
                    {
                        ...
                        if(number == 1)
                        {
                            ...
                            db_plc_helper.MultithreadExecuteNonQuery(sqltext);
                            writelog("RealTime_Car_Bag", "B道罐包总重写入plc！", "log");
                        }
                        else
                        {
                            Program.GB_data_flag = 1005;
                    ...
                    else
                    {
                        ...
                        db_plc_helper.MultithreadExecuteNonQuery(sqltext);
                        Program.GB_data_flag = 1005;
                        writelog("RealTime_Car_Bag", "B道罐包未查询到总重！", "log");
                    }
                ...
                else if(LGB_B_getpow == true && ZT_data.GB_B_connect == false)
                {
                    ...
                    db_plc_helper.MultithreadExecuteNonQuery(sqltext);
                    Program.GB_data_flag = 1005;
                    writelog("RealTime_Car_Bag", "B道罐包总重清零！", "log");
                }
                else
                {
        ...
        public void ztlog()
        {
            if ((MdiParent.tl1.Get_iron || TB_weight_speed > 0.09))//&& ZT_data.TB_pos == false  
            {
                ...
                if (MdiParent.tl1.TL_station == "A")
                    log += Math.Round((ZT_data.GB_A_angle / 20), 2).ToString() + ",2号" + "," + MdiParent.tl1.Get_iron.ToString() + "," + ZT_data.TB_pos.ToString() + "," + GB_A_remind_weight + "," + hum_speed + "," + ZT_data.GB_A_speed + "," + MdiParent.atuomodelA.ZT_program_flag.ToString() + "," + Program.testcode ;
                else if (MdiParent.tl1.TL_station == "B")
                    log += Math.Round((ZT_data.GB_B_angle / 20), 2).ToString() + ",1号" + "," + MdiParent.tl1.Get_iron.ToString() + "," + ZT_data.TB_pos.ToString() + "," + GB_B_remind_weight + "," + hum_speed + "," + ZT_data.GB_B_speed + "," + MdiParent.atuomodelA.ZT_program_flag.ToString() + "," + Program.testcode ;
                else
