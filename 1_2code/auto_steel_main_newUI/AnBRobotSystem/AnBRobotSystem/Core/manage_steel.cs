...
namespace AnBRobotSystem.Core
{
    ...
    public class manage_steel
    {
        ...
        public bool get_TB_data(float aim_weight)
        {
            try
            {
                ...
                if (MdiParent.PLCdata1.ZT_data.TB_pos ==false)//在位时为false  设为true方便调试
                {
                    ...
                    {
                        ibag_flag = "NF";
                        need_ibag_weight = aim_weight - realtime_weight;
                        writelisview("罐包管理系统", "计算所需铁水", "log");
                        return true;
                    }
                    else
        ...
        public bool get_GB_data_manual(string GB_ID)
        {
            try
            {
                ...
                if (GB_ID == "A")
                {
                    ...
                    {
                        writelisview("手动罐数据", "2号位罐车得电！", "log");
                        //if(fish_num == (PLCdata.ZT_data.GB_A_num).ToString())
                        //{
                            writelisview("手动罐数据", "鱼雷罐号确认成功！", "log");
                            if(MdiParent.PLCdata1.ZT_data.GB_A_0_limt == false&& MdiParent.PLCdata1.ZT_data.GB_A_120_limt == true||fish_weight == fish_init_weight && fish_init_weight > 200|| MdiParent.PLCdata1.ZT_data.GB_A_angle < 600)
                            {
                                F_flag = "F";
                                fish_weight = AFinit_weight;
                                fish_init_weight = AFinit_weight;
                    ...
                    {
                        writelisview("手动罐数据", "2号位罐车失电0！", "log");
                        return false;
                    }
                }
                else
                {
                    ...
                    {
                        writelisview("手动罐数据", "1号位罐车得电！", "log");
                        //if (fish_num == (PLCdata.ZT_data.GB_B_num).ToString())
                       // {
                            writelisview("手动罐数据", "1号鱼雷罐号确认成功！", "log");
                            if (MdiParent.PLCdata1.ZT_data.GB_B_0_limt == false && MdiParent.PLCdata1.ZT_data.GB_B_120_limt == true|| fish_weight == fish_init_weight && fish_init_weight > 200|| MdiParent.PLCdata1.ZT_data.GB_B_angle<600)
                            {
                                F_flag = "F";
                                fish_weight = AFinit_weight;
                                fish_init_weight = AFinit_weight;
                                writelisview("手动罐数据", "1号鱼雷罐满罐！", "log");
                                return true;
                            }
                            else
                            {
                                ...
                                fish_weight = AF_mid_weight;
                                fish_init_weight = AFinit_weight;
                                writelisview("手动罐数据", "1号鱼雷罐不满罐！", "log");
                                return true;
                            }
                       // }
                        //else
                    ...
                    {
                        writelisview("手动罐数据", "1号位罐车失电0！", "log");
                        return false;
                    }
                }
