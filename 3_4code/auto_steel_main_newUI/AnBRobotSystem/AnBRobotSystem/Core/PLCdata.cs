...
namespace AnBRobotSystem.Core
{
    public struct plcdata
    {
        ...
        public bool B_ZT_start_signal;
        public bool B_MODEL_RESET;
    ...
    public class PLCdata
    {
        ...
        public void ztlog()
        {
            if ((MdiParent.tl1.Get_iron || TB_weight_speed > 0.09))//&& ZT_data.TB_pos == false  
            {
                ...
                string head = "时间,折铁增重速度,铁包总重,铁包液位,罐车倾角,折铁位置,铁流有无判断,铁包位置,罐包剩余重量,人工速度,自动速度,最终控速";
                string log = TB_weight_speed.ToString() + "," + ZT_data.TB_weight.ToString() + "," + ZT_data.TB_LIQID.ToString() + ",";
                if (MdiParent.tl1.TL_station == "A")
                    log += Math.Round((ZT_data.GB_A_angle / 20), 2).ToString() + ",4号" + "," + MdiParent.tl1.Get_iron.ToString() + "," + ZT_data.TB_pos.ToString() + "," + GB_A_remind_weight + "," + hum_speed + "," + ZT_data.GB_A_speed +"," +Program.testcode;
                else if (MdiParent.tl1.TL_station == "B")
                    log += Math.Round((ZT_data.GB_B_angle / 20), 2).ToString() + ",3号" + "," + MdiParent.tl1.Get_iron.ToString() + "," + ZT_data.TB_pos.ToString() + "," + GB_B_remind_weight + "," + hum_speed + "," + ZT_data.GB_B_speed + "," + Program.testcode;
                else
                    log += MdiParent.tl1.Get_iron.ToString() + "," + ZT_data.TB_pos.ToString() + "," + ZT_data.TB_pos.ToString() + "," + GB_B_remind_weight + "," + Program.testcode;
                my_log.WriteLogcsv(LogFile.Trace, log, head);
               // LogHelper.csvlog.Info(log);
            }
        }
        #endregion
    }
